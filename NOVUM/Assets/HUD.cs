        ...
        void OnGUI()
        {
            ...
            if (!manager.IsClientConnected() && !NetworkServer.active && manager.matchMaker == null)
            {
                if (noConnection)
                {
                    ...
                    if (GUI.Button(new Rect(xpos, ypos, 200, 300), "LAN Client(C)"))
                    {
                        manager.StartClient();
                    }

                    manager.networkAddress = GUI.TextField(new Rect(xpos + 250, ypos, 200, 300), manager.networkAddress);
                    ypos += spacing;
            ...
            {
                if (GUI.Button(new Rect(xpos, ypos, 200, 300), "Client Ready"))
                {
            ...
            if (NetworkServer.active || manager.IsClientConnected())
            {
                ...
                if (GUI.Button(new Rect(xpos, ypos, 200, 300), "Stop (X)"))
                {
                    manager.StopHost();
                }
                ypos += spacing;
